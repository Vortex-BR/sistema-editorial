...
class EditorialIntelligenceV3Graph:
    ...
    def _transition(self, state: V3PipelineState) -> V3PipelineState:
        if state.stage == V3Stage.content_contract:
            ...
            source_report = state.source_coverage_report or {}
            if source_report.get("status") == "passed" or bool(
                source_report.get("synthesis_ready")
            ):
                ...
                return self._block(
                    ...
                    code=str(
                        source_report.get("suggested_blocking_code")
