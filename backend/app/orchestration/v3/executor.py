...
class EditorialV3Executor:
    ...
    def __init__(
        ...
        self.information_coverage = InformationCoverageService()
        ...
        if state.stage == V3Stage.completed:
            ...
            await self.runtime.event(
                ...
                blocked_stage,
                {
                    ...
                    "error_code": state.blocking_code,
                    "claim_pipeline_diagnostics": state.research_metrics.get(
                        "claim_pipeline_diagnostics"
                    )
                    or {},
                    "claim_persistence_diagnostics": state.research_metrics.get(
                        "claim_persistence_diagnostics"
                    )
                    or {},
                },
    ...
    async def source_reader(self, state: V3PipelineState) -> V3PipelineState:
        ...
        updated_requirement_map = dict(state.source_requirement_map)
        ...
        for payload in state.raw_source_documents:
            ...
            requirements = updated_requirement_map.get(raw_key, [])
            for key in {
                raw_key,
                ...
                updated_requirement_map[key] = sorted(
                    set(updated_requirement_map.get(key, [])) | set(requirements)
                )
        ...
        state.source_requirement_map = updated_requirement_map
    ...
    async def targeted_source_recovery(self, state: V3PipelineState) -> V3PipelineState:
        information_mode = bool(state.information_recovery_tasks)
        intelligence_mode = (
            not information_mode and bool(state.intelligence_recovery_tasks)
        )
        recovery_mode = (
            "information"
            if information_mode
        ...
        current_round = (
            state.information_recovery_round
            if information_mode
            else state.intelligence_recovery_round
            if intelligence_mode
            else state.source_recovery_round
        )
        next_round = current_round + 1
        recovery_label = {
            "information": "informações obrigatórias",
            "intelligence": "inteligência editorial",
            "source_coverage": "fontes",
        }[recovery_mode]
        await self._stage(
        ...
        intent = CanonicalResearchIntent.from_contract(contract)
        max_rounds = (
            int(self._optional_flag("v3_max_information_recovery_rounds", 3))
            if information_mode
            else 2
            if intelligence_mode
            else int(self._flag("v3_max_source_recovery_rounds"))
        )
        if current_round >= max_rounds:
            if information_mode:
                state.information_recovery_exhausted = True
                state.blocking_code = (
                    state.blocking_code or "V3_INFORMATION_RECOVERY_EXHAUSTED"
                )
        ...
        active_recovery_tasks = (
            ...
            if information_mode
            else state.intelligence_recovery_tasks
            if intelligence_mode
            else []
        )
        recovery_entries = [
            dict(item) for item in active_recovery_tasks if isinstance(item, dict)
        ]
        ...
        deficient_ids = (
            ...
            if information_mode or intelligence_mode
        ...
        retargeted_existing_urls: set[str] = set()
        attempts: list[dict] = []
        failures: list[SearchProviderError] = []
        successful_attempts = 0
        executed = {
        ...
        # In information recovery mode the unit of work is the missing
        # requirement, not the task.  The old task-level loop executed only one
        # query for a node even when several distinct questions were missing.
        # That made recovery look active while silently skipping most gaps.
        work_items: list[tuple[ResearchTask, dict[str, Any], str]] = []
        if information_mode or intelligence_mode:
            # InformationCoverageService already orders the queue by criticality,
            # partial coverage and query variant. Preserve that order so a bounded
            # round tries the first query for many gaps before spending additional
            # slots on variants of the same requirement.
            ordered_entries = list(recovery_entries)
            seen_requirement_queries: set[tuple[str, str]] = set()
            for entry in ordered_entries:
                task = task_by_id.get(str(entry.get("task_id") or ""))
                query = " ".join(str(entry.get("query") or "").split())[:280]
                requirement_id = str(entry.get("requirement_id") or "")
                if task is None or not query:
                    continue
                pair = (requirement_id, query.casefold())
                if pair in seen_requirement_queries:
                    continue
                seen_requirement_queries.add(pair)
                if query in set(executed.get(task.task_id, [])):
                    continue
                work_items.append((task, entry, query))
                if len(work_items) >= maximum_round_queries:
                    break
        else:
            for task in tasks[:maximum_round_queries]:
                ...
                if query:
                    work_items.append((task, {}, query))

        targeted_task_ids: list[str] = []
        targeted_requirement_ids: list[str] = []
        for task, recovery_entry, query in work_items:
            if budget.exhaustion_reason(include_logical=True):
                break
            targeted_task_ids.append(task.task_id)
            requirement_id = str(recovery_entry.get("requirement_id") or "")
            if requirement_id:
                targeted_requirement_ids.append(requirement_id)
            executed.setdefault(task.task_id, []).append(query)
            question = str(recovery_entry.get("question") or task.research_goal)
            result = await coordinator.search(
                query=query,
                topic=contract.topic,
                question=question,
            ...
            for document in result.documents:
                ...
                if information_mode and requirement_id:
                    ...
                    state.source_requirement_map[key] = sorted(
                        previous_requirements | {requirement_id}
                    )
                    # A provider may return a source that was already read for a
                    # different information unit. Retargeting that source is useful
                    # progress: the claim extractor must run again with the newly
                    # explicit requirement instead of treating the duplicate URL as
                    # an empty recovery round.
                    if (
                        key in existing_urls
                        and requirement_id not in previous_requirements
                    ):
                        retargeted_existing_urls.add(key)
                if (
        ...
        recovery_progress = bool(new_documents or retargeted_existing_urls)
        exhausted = bool(
            not recovery_progress
            and (next_round >= max_rounds or budget_reason or no_more_provider)
        )
        if information_mode:
            state.information_recovery_round = next_round
            state.information_recovery_exhausted = exhausted
            if recovery_progress:
                state.information_recovery_exhausted = False
                state.information_coverage_report = None
                state.blocking_code = None
                state.blocking_reason = None
        elif intelligence_mode:
        ...
        if exhausted:
            ...
            state.blocking_reason = (
                "A recuperação por informação obrigatória não encontrou novas "
                "evidências após esgotar as consultas seguras."
                if information_mode
                else "A recuperação orientada pelo Motor de Inteligência Editorial "
        ...
        state.research_metrics = {
            **state.research_metrics,
            ...
            f"{metric_prefix}_retargeted_existing_count": len(
                retargeted_existing_urls
            ),
            f"{metric_prefix}_attempts": [
                *state.research_metrics.get(f"{metric_prefix}_attempts", []),
                *attempts,
            ][-240:],
            "last_recovery_mode": recovery_mode,
        ...
        await self.runtime.event(
            self.project.id,
            self.pipeline_run.id,
            "v3.information.recovery_completed"
            if information_mode
            else "v3.intelligence.recovery_completed"
            if intelligence_mode
            ...
            "targeted_source_recovery",
            {
                "mode": recovery_mode,
                "round": next_round,
                "targeted_task_ids": sorted(set(targeted_task_ids)),
                "targeted_requirement_ids": sorted(set(targeted_requirement_ids)),
                "new_candidate_count": len(new_documents),
                "retargeted_existing_count": len(retargeted_existing_urls),
            ...
            idempotency_key=(
                f"v3.information.recovery:{next_round}"
                if information_mode
                else f"v3.intelligence.recovery:{next_round}"
                if intelligence_mode
    ...
    async def knowledge_synthesizer(self, state: V3PipelineState) -> V3PipelineState:
        ...
        advisory_claim_target = int(
            self._optional_flag("v3_min_approved_claims", 18)
        )
        claim_diagnostics = await self.artifacts.claim_pipeline_diagnostics()
        evidence_records = await self.artifacts.claim_coverage_records()
        information_report = self.information_coverage.evaluate(
            topic=contract.topic,
            plan=plan,
            evidence_records=evidence_records,
            minimum_overall_ratio=float(
                self._optional_flag("v3_min_information_coverage_ratio", 0.85)
            ),
        )

        # A dense batch may still miss one specific information unit even when it
        # extracted useful facts for the rest of the node. Retry only the missing
        # units against the already-read documents before spending another search
        # request. This is the information-by-information recovery path.
        missing_requirement_ids = set(
            information_report.critical_missing_requirement_ids
        ) | set(information_report.supporting_missing_requirement_ids)
        if information_report.status != "passed" and missing_requirement_ids:
            missing_task_ids = {
                item.task_id
                for item in information_report.requirement_reports
                if item.requirement_id in missing_requirement_ids
            }
            retry_tasks = [
                task for task in plan.tasks if task.task_id in missing_task_ids
            ]
            if retry_tasks:
                await self._extract_claims_for_tasks(
                    state=state,
                    ...
                    persisted_plan=persisted_plan,
                    force_per_document=True,
                    only_requirement_ids=missing_requirement_ids,
                )
        ...
        coverage_payload = information_report.as_payload(
            include_recovery_tasks=False
        )
        state.information_coverage_report = coverage_payload
        state.research_metrics = {
            **state.research_metrics,
            "claim_pipeline_diagnostics": claim_diagnostics,
            "approved_claim_count": len(claims),
            # Kept only as a density diagnostic for backwards-compatible
            # dashboards. It is no longer a blocking quota.
            "advisory_claim_target": advisory_claim_target,
            "information_coverage": coverage_payload,
        }
        await self.runtime.event(
            self.project.id,
            self.pipeline_run.id,
            ...
            {
                **claim_diagnostics,
                "approved_canonical_claim_count": len(claims),
                "advisory_claim_target": advisory_claim_target,
                "claim_quota_is_blocking": False,
                "persistence_diagnostics": state.research_metrics.get(
                    "claim_persistence_diagnostics"
                )
                or {},
                "tasks_without_documents": state.research_metrics.get(
                    "claim_extraction_tasks_without_documents"
                )
                or [],
                "empty_batch_task_ids": state.research_metrics.get(
                    "claim_extraction_empty_batch_task_ids"
                )
                or [],
            },
            idempotency_key=(
                "v3.claims.evaluated.v390:"
                f"{state.information_recovery_round}"
            ),
            context=self._stage_context,
        )
        await self.runtime.event(
            self.project.id,
            self.pipeline_run.id,
            "v3.information.coverage_evaluated",
            "knowledge_synthesizer",
            {
                **coverage_payload,
                "recovery_task_count": len(information_report.recovery_tasks),
                "recovery_task_preview": [
                    dict(item)
                    for item in information_report.recovery_tasks[:20]
                ],
            },
            idempotency_key=(
                "v3.information.coverage:"
                f"{state.information_recovery_round}:"
                f"{information_report.overall_coverage_ratio}"
            ),
            context=self._stage_context,
        )

        if information_report.status != "passed":
            missing_reports = [
                item
                for item in information_report.requirement_reports
            ...
            state.knowledge_gaps = [
                {
                    ...
                    "gap_type": (
                        "procedure_without_support"
                        if is_procedural
                        else "overbroad_question"
                    ),
                    "description": (
                        f"[{item.requirement_id}] {item.description} "
                        f"— estado: {item.status}; motivos: "
                        + ", ".join(item.reason_codes)
                    ),
                    "essential": item.critical,
                    "status": "researching"
                    if not state.information_recovery_exhausted
                    else "blocked",
                }
                for item in missing_reports[:40]
            ]
            recovery_tasks = list(information_report.recovery_tasks)
            if (
                recovery_tasks
                and state.information_recovery_round
                < int(
                    self._optional_flag(
                ...
                max_rounds = int(
                    self._optional_flag("v3_max_information_recovery_rounds", 3)
                )
                per_round = int(
                    self._optional_flag(
                        "v3_max_information_recovery_queries_per_round", 8
                ...
                remaining_rounds = max(
                    1, max_rounds - state.information_recovery_round
                )
                recovery_capacity = max(1, remaining_rounds * per_round)
                state.information_recovery_tasks = recovery_tasks[:recovery_capacity]
                state.blocking_code = None
                state.blocking_reason = None
                return state
            ...
            state.information_recovery_tasks = []
            state.information_recovery_exhausted = True
            state.blocking_code = str(
                information_report.suggested_blocking_code
                or "V3_INFORMATION_COVERAGE_INCOMPLETE"
            )
            missing_labels = "; ".join(
                f"{item.requirement_id}: {item.description}"
                for item in missing_reports[:8]
            )
            state.blocking_reason = (
                "A pesquisa foi avaliada por informação necessária, não por uma "
                "contagem arbitrária de claims. Permaneceram requisitos sem "
                f"evidência aprovada após a recuperação: {missing_labels}."
            )
            return state
    ...
    async def _extract_claims_for_tasks(
        ...
        *,
        ...
        persistence_diagnostics = {
            str(key): int(value)
            for key, value in dict(
                state.research_metrics.get("claim_persistence_diagnostics") or {}
            ).items()
            if isinstance(value, (int, float))
        }
        persisted_by_task = {
            str(key): int(value)
            for key, value in dict(
                state.research_metrics.get("claim_extraction_persisted_by_task")
                or {}
            ).items()
            if isinstance(value, (int, float))
        }
        candidate_count_by_task = {
            str(key): int(value)
            for key, value in dict(
                state.research_metrics.get("claim_extraction_candidate_count_by_task")
                or {}
            ).items()
            if isinstance(value, (int, float))
        }
        empty_batch_task_ids: set[str] = set(
            state.research_metrics.get("claim_extraction_empty_batch_task_ids") or []
        )
        tasks_without_documents: set[str] = set(
            state.research_metrics.get("claim_extraction_tasks_without_documents")
            or []
        )
        ...
        def document_for_extraction(
            document: StructuredSourceDocument, task: ResearchTask
        ) -> dict[str, Any]:
            payload = self._document_for_agent(document, task.research_goal)
            mapped: set[str] = set()
            for key in {
                canonicalize_url(str(document.url)),
                canonicalize_url(str(document.canonical_url)),
            }:
                mapped.update(state.source_requirement_map.get(key, []))
            allowed = {
                item.requirement_id
                for item in requirements_for_task(task)
                if only_requirement_ids is None
                or item.requirement_id in only_requirement_ids
            }
            payload["target_requirement_ids"] = sorted(mapped & allowed)
            return payload
        ...
        async def extract_batch(
            ...
            *,
            ...
            task_payload = task.model_copy(
                update={"coverage_requirements": active_requirements}
            ...
            output = await self._agent_call(
                ...
                input_json={
                    ...
                    "documents": [
                        document_for_extraction(item, task) for item in batch
                    ],
                },
                prompt=(
                    "Você é o pesquisador factual da Editorial Intelligence V3. Trabalhe requisito por requisito, "
                    "não por quantidade global de claims. Para cada coverage_requirement que possuir suporte "
                    "literal nos documentos, extraia uma ou mais afirmações atômicas e inclua exatamente o "
                    "requirement_id correspondente em coverage_requirement_ids. Não associe um claim a um requisito "
                    "que ele não responda diretamente. exact_quote deve ser um trecho exato e localizável. Reúna "
                    "afirmações semanticamente equivalentes de fontes independentes no mesmo support_group; produza "
                    "um registro por fonte e reutilize exatamente o mesmo support_group. Não retorne claims=[] quando "
                    "houver ao menos uma afirmação literal que responda a qualquer requisito. Registre em "
                    "unresolved_questions os requisitos sem evidência. critical=true deve ser usado apenas para uma "
                    "afirmação cuja ausência comprometa segurança, decisão central ou conclusão principal; uma tarefa "
                    "crítica não torna todas as afirmações críticas. Conteúdo comparison_only só pode registrar "
                    "comparação, limitação ou erro relatado e nunca sustenta conclusão absoluta. Não invente método, "
                    "parâmetro, autoria, URL ou causalidade. Use conditional quando a resposta depender de contexto. "
                    f"Tarefa: {task.research_goal}"
                ),
        ...
        async def persist_extraction(
            ...
            count = 0
            candidate_count_by_task[task.task_id] = int(
            ...
            for candidate in extraction.claims:
                try:
                    if allowed_roles and candidate.evidence_role not in allowed_roles:
                        persistence_diagnostics["task_evidence_role_mismatch"] = int(
                            persistence_diagnostics.get(
                                "task_evidence_role_mismatch", 0
                            )
                        ) + 1
                        continue
                    supplied_requirement_ids = list(
                        dict.fromkeys(candidate.coverage_requirement_ids or [])
                    )
                    valid_requirement_ids = [
                        item for item in supplied_requirement_ids if item in requirement_ids
                    ]
                    invalid_requirement_count = len(supplied_requirement_ids) - len(
                        valid_requirement_ids
                    )
                    if invalid_requirement_count:
                        persistence_diagnostics["unknown_coverage_requirement_id"] = int(
                            persistence_diagnostics.get(
                                "unknown_coverage_requirement_id", 0
                            )
                        ) + invalid_requirement_count
                    if not valid_requirement_ids:
                        valid_requirement_ids = infer_requirement_ids(
                            task,
                        ...
                        if only_requirement_ids is not None:
                            valid_requirement_ids = [
                                ...
                                if item in only_requirement_ids
                            ]
                        if valid_requirement_ids:
                            persistence_diagnostics[
                                "coverage_requirement_inferred"
                            ] = int(
                                persistence_diagnostics.get(
                                    "coverage_requirement_inferred", 0
                                )
                            ) + 1
                        else:
                            persistence_diagnostics[
                                "claim_without_coverage_requirement"
                            ] = int(
                                persistence_diagnostics.get(
                                    "claim_without_coverage_requirement", 0
                                )
                            ) + 1
                    ...
                    if document is None:
                        persistence_diagnostics["candidate_document_not_found"] = int(
                            persistence_diagnostics.get(
                                "candidate_document_not_found", 0
                            )
                        ) + 1
                        continue
                    ...
                    if source_row is None:
                        persistence_diagnostics["source_row_not_found"] = int(
                            persistence_diagnostics.get("source_row_not_found", 0)
                        ) + 1
                        continue
                    ...
                    normalized = candidate.model_copy(
                        update={
                            ...
                            "knowledge_node_id": task.knowledge_node_id,
                            "coverage_requirement_ids": valid_requirement_ids,
                            # A critical research task does not make every atomic
                            # sentence publication-critical.  Blanket promotion
                            # forced every bundle to require two independent
                            # sources and could reduce a healthy run to zero
                            # approved claims.  The extractor must explicitly
                            # mark the individual claim as critical.
                            "critical": bool(task.critical and candidate.critical),
                        }
                    )
        ...
        async def extract_isolated_documents(
            task: ResearchTask,
            node,
            task_documents: list[StructuredSourceDocument],
            *,
            base_attempt: int,
        ) -> int:
            isolated_count = 0
            for index, document in enumerate(task_documents, start=1):
                try:
                    ...
                    continue
                isolated_count += await persist_extraction(
        ...
        for task in tasks:
            ...
            if node is None:
                continue
            if only_requirement_ids is not None and not any(
                item.requirement_id in only_requirement_ids
                for item in requirements_for_task(task)
            ):
                continue
            task_documents = []
            for document in documents:
                if (
                    only_document_ids is not None
                    and document.document_id not in only_document_ids
                ):
                    continue
                ...
                for key in {
                    ...
                    assigned_task_ids.update(state.source_task_map.get(key, []))
            ...
            if not task_documents:
                tasks_without_documents.add(task.task_id)
                continue
            base_attempt = 1 if only_document_ids is None else 2
            if force_per_document:
                task_count = await extract_isolated_documents(
                    task, node, task_documents, base_attempt=base_attempt
                )
                persisted_count += task_count
                persisted_by_task[task.task_id] = int(
                    persisted_by_task.get(task.task_id, 0)
                ) + task_count
                continue
            try:
                extraction = await extract_batch(
                ...
                if extraction.claims:
                    task_count = await persist_extraction(
                        task, node, task_documents, extraction
                    )
                elif len(task_documents) > 1:
                    # Some models return an empty list when a multi-document
                    # prompt is dense even though each source contains usable
                    # evidence.  Treat empty output as a recoverable extraction
                    # condition and retry source-by-source.
                    empty_batch_task_ids.add(task.task_id)
                    task_count = await extract_isolated_documents(
                        task, node, task_documents, base_attempt=base_attempt
                    )
                else:
        ...
        state.research_metrics = {
            **state.research_metrics,
            ...
            "claim_extraction_persisted_by_task": persisted_by_task,
            "claim_extraction_candidate_count_by_task": candidate_count_by_task,
            "claim_extraction_empty_batch_task_ids": sorted(empty_batch_task_ids),
            "claim_extraction_tasks_without_documents": sorted(
                tasks_without_documents
            ),
            "claim_persistence_diagnostics": persistence_diagnostics,
