...
class V3PipelineState(BaseModel):
    ...
    information_recovery_round: int = Field(default=0, ge=0, le=5)
    information_recovery_exhausted: bool = False
    information_recovery_tasks: list[dict] = Field(default_factory=list)
    ...
    source_requirement_map: dict[str, list[str]] = Field(default_factory=dict)
    ...
    information_coverage_report: dict | None = None
    ...
    def resume_invariant_errors(
        ...
        if self.stage in writer_or_later:
            ...
            if (self.information_coverage_report or {}).get("status") != "passed":
                errors.append(
                    f"stage {self.stage.value} requires passed information coverage"
                )
