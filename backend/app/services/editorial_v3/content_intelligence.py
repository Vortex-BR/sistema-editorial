...
class ContentIntelligenceEngine:
    ...
    def augment_research_plan(
        ...
        V3.9 research tasks already contain requirement-level queries derived
        from the contract.  Canonical intelligence questions therefore fill only
        unused slots and never displace executable requirement queries.
        """
        ...
        for task in plan.tasks:
            ...
            legacy_queries = list(dict.fromkeys(task.queries))[:6]
            if task.coverage_requirements:
                # Coverage-first tasks already carry requirement-specific queries.
                # Preserve those executable queries and use canonical questions only
                # when the planner left spare capacity.
                queries = list(legacy_queries)
                ...
                # Backward compatibility for legacy/checkpointed plans that do not
                # expose information requirements: reserve canonical-question slots
                # so six generic queries cannot crowd out the editorial question map.
                reserved = min(3, max(2, len(intelligence_queries)))
                legacy_capacity = max(0, 6 - reserved)
                queries = legacy_queries[:legacy_capacity]
                for query in intelligence_queries:
                    if len(queries) >= 6:
                        break
                    if query not in queries:
                        queries.append(query)
                for query in legacy_queries[legacy_capacity:]:
                    if len(queries) >= 6:
                        break
                    if query not in queries:
                        queries.append(query)
            ...
            rationale_suffix = (
                ...
                "para as perguntas críticas sem substituir consultas do contrato."
            )
