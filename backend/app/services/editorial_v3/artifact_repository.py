...
def _bump(diagnostics: dict[str, int] | None, key: str, amount: int = 1) -> None:
    """Increment a safe claim-persistence diagnostic counter when requested."""
...
class V3ArtifactRepository:
        ...
        if existing is None:
            ...
            for index, task in enumerate(plan.tasks, start=1):
                question = ResearchQuestion(
                    ...
                    semantic_terms=[
                        task.evidence_role.value,
                        task.knowledge_node_id,
                        *(task.method_hint and [task.method_hint] or []),
    ...
    async def claim(
        ...
        *,
        ...
        # ``support_group`` comes from a model and is not a trustworthy identity.
        # Separate extraction calls frequently describe the same fact with a
        # different slug, which used to leave every critical claim in a
        # single-source bundle and produced zero approved claims.  Reuse an
        # existing group only when deterministic semantic safeguards agree on
        # node, evidence role, numbers, negation and lexical direction.
        prior_candidates = list(
            (
                await self.db.scalars(
                    select(V3KnowledgeClaimRecord).where(
                        V3KnowledgeClaimRecord.pipeline_run_id
                        == self.pipeline_run_id,
                        V3KnowledgeClaimRecord.knowledge_node_key
                        == candidate.knowledge_node_id,
                        V3KnowledgeClaimRecord.evidence_role
                        == candidate.evidence_role.value,
                    )
                )
            ).all()
        )
        for prior in sorted(prior_candidates, key=lambda item: item.support_group):
            compatible, _reason = support_group_compatible(
                prior.claim_text, candidate.claim_text
            )
            if compatible and prior.support_group != candidate.support_group:
                candidate = candidate.model_copy(
                    update={"support_group": prior.support_group}
                )
                _bump(diagnostics, "support_group_reconciled")
                break
        ...
        if existing is not None:
            validation = dict(existing.validation_json or {})
            merged_requirements = list(
                dict.fromkeys(
                    [
                        *list(validation.get("coverage_requirement_ids") or []),
                        *list(candidate.coverage_requirement_ids or []),
                    ]
                )
            )
            if merged_requirements:
                validation["coverage_requirement_ids"] = merged_requirements
                existing.validation_json = validation
                await self.db.flush()
            _bump(diagnostics, "existing_claim_reused")
            return existing
        ...
        if fact is None:
            _bump(diagnostics, "fact_ledger_rejected")
            return None
        ...
        self.db.add(row)
        await self.db.flush()
        _bump(diagnostics, "claim_persisted")
        return row
    ...
    async def approve_claim_bundles(
        ...
        for support_group, claims in grouped.items():
            ...
            eligible_claims: list[V3KnowledgeClaimRecord] = []
            ignored_claims: list[V3KnowledgeClaimRecord] = []
            for item in claims:
            ...
            for index, left in enumerate(eligible_claims):
                for right in eligible_claims[index + 1 :]:
            ...
            critical = any(item.critical for item in eligible_claims)
            comparison = bool(eligible_claims) and all(
                item.evidence_role in {"comparison", "limitation", "common_error"}
                for item in eligible_claims
            )
            absolute = any(
                item.conclusion_status == ConclusionStatus.confirmed.value
                for item in eligible_claims
            )
            ...
            valid_conclusion = bool(eligible_claims) and not any(
            ...
            for item in claims:
                ...
                item.validation_json = {
                    **(item.validation_json or {}),
                    ...
                    "ignored_non_evidence_claim_count": len(ignored_claims),
    ...
    async def claim_pipeline_diagnostics(self) -> dict[str, object]:
        """Summarize why claim extraction/approval did or did not progress."""
        ...
        for row in rows:
            ...
            decision = dict(validation.get("bundle_decision") or {})
            for blocker in decision.get("blockers") or []:
                blocker_counts[str(blocker)] += 1
            document = documents.get(row.source_document_id)
            if document is not None:
                usage_counts[str(document.usage_policy)] += 1
        return {
            "total_claim_records": len(rows),
            "approved_claim_records": sum(bool(row.approved) for row in rows),
            "distinct_support_groups": len({row.support_group for row in rows}),
            "bundle_blocker_counts": dict(sorted(blocker_counts.items())),
            "source_usage_counts": dict(sorted(usage_counts.items())),
        }

    async def claim_coverage_records(self) -> list[dict[str, object]]:
        """Return claim/source facts required by the information coverage gate.

        Coverage requirement IDs live in ``validation_json`` to remain backwards
        compatible with the current database schema.  This method also exposes
        raw rejected claims so diagnostics can distinguish extraction failure
        from evidence-policy failure.
        """
            ...
            if document is not None:
                try:
                    assessment = SourceAssessment.model_validate(
                        document.assessment_json
                    )
                except ValidationError:
                    # Legacy or partially-written assessment payloads must not
                    # crash the coverage gate. They remain visible as extracted
                    # claims but cannot count as independent/authoritative support
                    # until a valid source assessment is available.
                    assessment = None
                source_host = (
                    urlsplit(document.canonical_url)
                    .netloc.casefold()
                    .removeprefix("www.")
                )
            result.append(
                {
                    ...
                    "knowledge_node_id": row.knowledge_node_key,
                    "evidence_role": row.evidence_role,
                    "claim_text": row.claim_text,
                    "approved": bool(row.approved),
                    "coverage_requirement_ids": list(
                        validation.get("coverage_requirement_ids") or []
                    ),
                    "source_host": source_host,
                    "independent_source": bool(
                        assessment
                        and assessment.counts_toward_independent_source_diversity
                    ),
                    "authoritative_source": bool(
                        assessment and assessment.eligible_for_primary_evidence
                    ),
                    "usage_policy": (
                        assessment.usage_policy.value if assessment else None
                    ),
                    "bundle_blockers": list(decision.get("blockers") or []),
                }
            )
        return result
