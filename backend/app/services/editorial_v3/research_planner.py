...
def _requirement_terms(value: str, *, limit: int = 10) -> list[str]:
    words = re.findall(r"[A-Za-zÀ-ÿ0-9%+-]{3,}", value or "")
    ignored = {
        "para", "como", "sobre", "entre", "quando", "onde", "qual", "quais",
        "deve", "devem", "pode", "podem", "com", "sem", "uma", "das", "dos",
        "the", "and", "with", "from", "that", "this", "what", "when", "how",
    }
    result: list[str] = []
    for word in words:
...
def _information_units(value: str, *, limit: int = 320) -> list[str]:
    """Split compound contract text without discarding later facts.

    Knowledge-contract fields are usually atomic, but imported/custom contracts
    may place several requirements in one sentence separated by punctuation.
    Coverage must retain each unit instead of truncating everything after the
    first full stop.
    """
    ...
    result: list[str] = []
    for part in parts:
        clean = part.strip(" -–—;:,.|")[:limit].rstrip()
        if clean and clean not in result:
            result.append(clean)
    return result


def _initial_requirement_queries(
    *,
    subject: str,
    requirements: list[ResearchCoverageRequirement],
    primary_role: EvidenceRole,
    technical_terms: str,
    limit: int = 6,
) -> list[str]:
    """Create a diversified first-pass query set from exact information units.

    Broad task-level queries are useful for discovery, but they cannot guarantee
    that the search engine ever sees a specific decision criterion, completion
    signal or troubleshooting question.  This scheduler keeps one broad query
    and then samples critical requirements across their semantic types before
    adding an English-language evidence query for international recall.
    """
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            not item.critical,
            {
                ...
                "decision": 2,
                "completion": 3,
            }.get(item.requirement_type, 9),
            item.requirement_id,
        ),
    )
    selected: list[ResearchCoverageRequirement] = []
    seen_types: set[str] = set()
    for item in ordered:
        if item.requirement_type in seen_types:
            continue
        selected.append(item)
        seen_types.add(item.requirement_type)
        if len(selected) >= 3:
            break
    for item in ordered:
        if item in selected:
    ...
    queries = [
        _query(
            subject,
            technical_terms,
            _ROLE_TERMS[primary_role],
            "evidência revisão protocolo",
        )
    ]
    # Reserve one slot for international recall. The previous implementation
    # appended the English query after six Portuguese queries, so it was always
    # truncated exactly when the task had several information types.
    targeted_limit = max(0, limit - 2)
    for item in selected[:targeted_limit]:
        role_terms = " ".join(
            _ROLE_TERMS.get(role, role.value.replace("_", " "))
            for role in item.evidence_roles[:2]
        )
        queries.append(
            _query(subject, item.description, role_terms, "fonte técnica")
        )
    if selected and len(queries) < limit:
        queries.append(
            _query(
                subject,
                selected[0].description,
                "study review guideline evidence",
            )
        )
    return list(dict.fromkeys(item for item in queries if item))[:limit]


def _coverage_requirements(node, *, task_id: str) -> list[ResearchCoverageRequirement]:
    """Expand one knowledge node into auditable information units.

    The previous pipeline measured success by a global number of claims.  These
    requirements preserve the actual editorial questions, required knowledge,
    decisions and completion criteria so the gate can say exactly what is
    covered and what still needs research.
    """

    raw: list[tuple[str, str]] = []
    for requirement_type, values in (
        ("knowledge", node.required_knowledge),
        ("decision", node.required_decisions),
        ("completion", node.completion_criteria),
    ):
        for value in values:
            raw.extend(
                (requirement_type, unit)
                for unit in _information_units(value)
            )
    raw.extend(("question", unit) for unit in _information_units(node.central_question))

    seen: set[str] = set()
    requirements: list[ResearchCoverageRequirement] = []
    node_roles = list(node.required_evidence_roles) or [EvidenceRole.definition]
    for requirement_type, description in raw:
        clean = re.sub(r"\s+", " ", description).strip()[:320].rstrip()
        signature = re.sub(r"[^a-z0-9]+", " ", clean.casefold()).strip()
        if not clean or signature in seen:
            continue
        seen.add(signature)

        preferred_roles = list(node_roles)
        if requirement_type == "decision":
            decision_roles = [
                role
            ...
            preferred_roles = decision_roles or preferred_roles
        elif requirement_type == "completion":
            completion_roles = [
                role
                for role in node_roles
                if role
                in {
                    ...
                    EvidenceRole.final_outcome,
                    EvidenceRole.correction,
                }
            ]
            preferred_roles = completion_roles or preferred_roles

        index = len(requirements) + 1
        requirements.append(
            ResearchCoverageRequirement(
                requirement_id=f"{task_id}_req_{index:02d}",
                requirement_type=requirement_type,
                description=clean,
                evidence_roles=preferred_roles[:12],
                critical=node.importance.value == "core",
                minimum_approved_claims=1,
                # Every critical information unit must be corroborated, not merely
                # the task as a whole. This prevents two unrelated sources in the
                # same node from making an unsupported critical answer look safe.
                minimum_independent_sources=(
                    2 if node.importance.value == "core" else 1
                ),
                query_terms=_requirement_terms(clean),
            )
        )
    ...
    def build(
        ...
        *,
        ...
        for node in contract.nodes:
            ...
            coverage_requirements = _coverage_requirements(node, task_id=task_id)
            focus = _short_phrase(
                next(iter(node.required_knowledge), "") or node.central_question,
                limit=100,
            )
            queries = _initial_requirement_queries(
                subject=search_subject,
                requirements=coverage_requirements,
                primary_role=primary_role,
                technical_terms=_query(focus, vocabulary["technical"]),
                limit=5,
            )
            if len(roles) > 1:
