...
class ResearchCoverageRequirement(V3OpenAIStrictOutput):
    """One information unit that must be supported before drafting.

    The research pipeline is evaluated against these units instead of a global
    quota of claims.  IDs are deterministic and remain stable across retries so
    extraction, recovery and diagnostics can refer to the same missing fact.
    """

    requirement_id: str = Field(pattern=r"^[a-z][a-z0-9_]{2,119}$")
    requirement_type: Literal["question", "knowledge", "decision", "completion"]
    description: str = Field(min_length=8, max_length=1000)
    evidence_roles: list[EvidenceRole] = Field(min_length=1, max_length=12)
    critical: bool = False
    minimum_approved_claims: int = Field(default=1, ge=1, le=4)
    minimum_independent_sources: int = Field(default=1, ge=1, le=3)
...
class ResearchTask(V3OpenAIStrictOutput):
    ...
    rationale: str = Field(min_length=10, max_length=1000)
    coverage_requirements: list[ResearchCoverageRequirement] = Field(
        default_factory=list, max_length=100
    )

    @model_validator(mode="after")
    def unique_coverage_requirements(self):
        ids = [item.requirement_id for item in self.coverage_requirements]
        if len(ids) != len(set(ids)):
            raise ValueError("Research coverage requirement IDs must be unique")
        return self
...
class ExtractedKnowledgeClaimCandidate(V3OpenAIStrictOutput):
    ...
    coverage_requirement_ids: list[str] = Field(default_factory=list, max_length=20)
