...
class Settings(BaseSettings):
    ...
    # Retained as an advisory density target for dashboards. V3.9 no longer
    # blocks the pipeline on a global claim quota; it gates exact information
    # requirements derived from the knowledge contract.
    v3_min_approved_claims: int = Field(default=18, ge=1, le=100)
    v3_min_information_coverage_ratio: float = Field(
        default=0.85, ge=0.5, le=1.0
    )
    v3_max_information_recovery_rounds: int = Field(default=3, ge=1, le=5)
    v3_max_information_recovery_queries_per_round: int = Field(
        default=8, ge=1, le=24
    )
