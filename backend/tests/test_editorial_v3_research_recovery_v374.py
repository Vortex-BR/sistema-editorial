...
@pytest.mark.asyncio
async def test_claim_extraction_recovers_empty_batch_uses_both_url_aliases_and_preserves_atomic_criticality():
    contract = KnowledgeContractBuilder().build(
        KnowledgeContractInput(
            topic="germinação de sementes em papel-toalha",
            reader_start_state="Leitor sem clareza sobre o processo.",
            reader_final_state="Leitor capaz de reconhecer o processo.",
            article_promise="Explicar o processo com evidências verificáveis.",
            scope_limit="O conteúdo termina na confirmação do resultado inicial.",
        )
    )
    plan = V3ResearchPlanningService().build(contract, max_tasks=100)
    task = plan.tasks[0].model_copy(update={"critical": True})
    original_url = "https://science-one.example/original"
    canonical_url = "https://science-one.example/canonical"
    document = _document(
        original_url,
        title="Fonte científica sobre absorção de água",
        text="A absorção de água inicia a ativação metabólica da semente.",
        role=SourceRole.scientific_primary,
        allowed_roles=[task.evidence_role],
    ).model_copy(update={"canonical_url": canonical_url})
    second = _document(
        "https://science-two.example/claim",
        title="Segunda fonte científica",
        text="A disponibilidade de água permite a retomada metabólica da semente.",
        role=SourceRole.scientific_primary,
        allowed_roles=[task.evidence_role],
    )
    ...
    executor.artifacts = SimpleNamespace(claim=AsyncMock(return_value=SimpleNamespace()))
    executor._flag = lambda name: 6 if name == "v3_max_documents_per_research_task" else 0
    executor._document_for_agent = lambda item, _goal: {
    ...
    async def agent_call(**kwargs):
        urls = [item["url"] for item in kwargs["input_json"]["documents"]]
        calls.append(urls)
        if len(urls) > 1:
            return {
                "claims": [],
        ...
        return {
            "claims": [
                {
                    "claim_key": "water_supports_metabolic_restart",
                    "support_group": "water_metabolic_restart",
                    "source_url": urls[0],
                    "knowledge_node_id": task.knowledge_node_id,
                    "evidence_role": task.evidence_role.value,
                    "claim_text": "A água participa da retomada metabólica da semente.",
                    "exact_quote": "A absorção de água" if "canonical" in urls[0] else "A disponibilidade de água",
                    "source_locator": "section:1",
                    "method_labels": [],
                    "conditions": [],
                    "applicability": [],
    ...
    state = V3PipelineState(
        ...
        pipeline_run_id=executor.pipeline_run.id,
        contract_id=uuid4(),
        # Deliberately map the redirected document only by its original URL.
        source_task_map={
            canonicalize_url(original_url): [task.task_id],
            canonicalize_url(str(second.canonical_url)): [task.task_id],
        },
    )
    persisted_plan = SimpleNamespace(
        questions_by_task_id={task.task_id: SimpleNamespace(id=uuid4())}
    ...
    assert all(len(item) == 1 for item in calls[1:])
    assert state.research_metrics["claim_extraction_empty_batch_task_ids"] == [
        task.task_id
    ]
    assert state.research_metrics["claim_extraction_persisted_by_task"][task.task_id] == 2
    first_call = executor.artifacts.claim.await_args_list[0]
    assert first_call.kwargs["candidate"].critical is False
    assert "diagnostics" in first_call.kwargs
