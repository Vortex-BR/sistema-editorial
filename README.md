...
> **Atualização V3.9.1 — Continuidade da recuperação de fontes:** corrige o bloqueio prematuro `V3_SOURCE_DIVERSITY_INSUFFICIENT`. O gate preliminar agora permite a extração quando todas as tarefas possuem fonte válida e a única pendência é diversidade; a exigência de duas fontes continua sendo aplicada por informação crítica. URLs já lidas e reaproveitadas para outra tarefa passam a contar como progresso real. Sem migration nova; o head permanece `0037`. Consulte `CHANGELOG_EDITORIAL_V3_9_1.md`, `RELATORIO_CORRECAO_DIVERSIDADE_FONTES_V3_9_1.md` e `VALIDATION_EDITORIAL_V3_9_1.md`.

> **Atualização V3.9 — Pesquisa orientada por cobertura:** substitui a cota global de claims por validação de cada pergunta, conhecimento, decisão e critério de conclusão do contrato. A descoberta passa a pesquisar informações específicas, o extrator vincula claims aos requisitos, requisitos críticos exigem corroboração independente e a recuperação ocorre informação por informação, inclusive reutilizando fontes já lidas. O front-end mostra exatamente o que está coberto e o que ainda falta. Sem migration nova; o head permanece `0037`. Consulte `CHANGELOG_EDITORIAL_V3_9.md`, `RELATORIO_REFATORACAO_PESQUISA_V3_9.md` e `VALIDATION_EDITORIAL_V3_9.md`.

> **Atualização V3.8.3 — Recuperação de claims e Fact Ledger:** corrige execuções que chegavam à síntese com fontes aprovadas, mas terminavam com `0 fatos coletados` e `V3_APPROVED_CLAIMS_INSUFFICIENT`. A extração agora considera URL original e canônica, recupera lotes vazios fonte por fonte, registra os motivos de descarte, reconcilia grupos semanticamente equivalentes e impede que uma fonte apenas comparativa contamine evidência factual válida. Sem migration nova; o head permanece `0037`. Consulte `CHANGELOG_EDITORIAL_V3_8_3.md` e `VALIDATION_EDITORIAL_V3_8_3.md`.

> **Atualização V3.8.2 — Logs técnicos no front-end:** adiciona uma aba administrativa de diagnóstico por projeto e execução, com busca, filtros, atualização automática, cópia e exportação JSON. Falhas técnicas passam a ser persistidas de forma sanitizada e independente na tabela `technical_error_logs`; o Alembic head agora é `0037`. Consulte `CHANGELOG_EDITORIAL_V3_8_2.md`, `VALIDATION_EDITORIAL_V3_8_2.md` e `RELATORIO_IMPLEMENTACAO_LOGS_V3_8_2.md`.

> **Atualização V3.8.1 — Idempotência das fontes:** corrige a colisão global de `document_id` que encerrava a etapa **Fontes** com `pk_v3_source_documents` quando uma fonte já usada aparecia em outra execução. Os registros agora recebem uma chave primária determinística por execução, o `INSERT` é protegido por `ON CONFLICT`, checkpoints antigos são reconciliados e o ID persistido é propagado até a extração de claims. Sem migration nova; o head permanece `0036`. Consulte `CHANGELOG_EDITORIAL_V3_8_1.md`, `VALIDATION_EDITORIAL_V3_8_1.md` e `RELATORIO_CORRECAO_EDITORIAL_V3_8_1.md`.
